  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <div style={{ fontSize: 13, color: '#7e93a0' }}>{fullDatoTekst(t)}</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14 }}>
        {dashKort.map((k) => (
          <button
            ...
            style={{ textAlign: 'left', background: '#fff', border: '1px solid #e1e8ec', borderRadius: 14, padding: '16px 18px', cursor: 'pointer' }}
          >
            ...
            <div style={{ fontSize: 12, color: '#7e93a0', marginTop: 2 }}>{k.sub}</div>
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 16 }}>
        <div style={{ background: '#fff', border: '1px solid #e1e8ec', borderRadius: 14, padding: '16px 18px' }}>
          ...
          {jobbarIdag.length === 0 && <div style={{ fontSize: 13, color: '#7e93a0' }}>Ingen vakter i dag.</div>}
              ...
              return (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                  ...
                  <div style={{ flex: 1, lineHeight: 1.2 }}>
                    ...
                    <div style={{ fontSize: 12, color: '#7e93a0' }}>{s.skift}</div>
                  </div>
                  <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 13, color: '#566570' }}>{s.start}–{s.slutt}</div>
                </div>
              );
            })}
          </div>
            ...
            {dashTimar.map(({ a, tot, pct }) => (
              <div key={a.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, marginBottom: 4 }}>
                  ...
                  <span style={{ color: '#7e93a0', fontFamily: "'JetBrains Mono'" }}>{fmt(tot)} / 37,5 t</span>
                </div>
                <div style={{ height: 8, background: '#eef2f4', borderRadius: 5, overflow: 'hidden' }}>
      ...
      <div style={{ background: '#0c2436', color: '#fff', borderRadius: 14, padding: '16px 18px' }}>
        ...
        {dashFerie.length === 0 && <div style={{ fontSize: 13, color: '#9db1bc' }}>Ingen registrert.</div>}
